...
class Ecu():
	...
	def consume_data(self, command):
		...
		x = 0
		while read_thread:
			incomingData = self.PORT.read(16) if self.enviroment == "production" else self.development(x)
...
# 				INTAKE_Value = self.convertToTemp(int(dataList[7]))
				...
				self.socketio.emit('ecuData', {'rpm': RPM_Value, 'kmh': KMH_Value, 'mph': MPH_Value, 'temp': TEMP_Value, 'batt': BATT_Value, 'turbo': TURBO_Value, 'tps': TPS_Value })
				time.sleep(0.002)
