...
from model.models import db, Alarm, AlarmSchema, AlarmType, AlarmTypeSchema, AlarmOutput, AlarmOutputSchema, ChannelOutput, Condition
...
class AlarmSetting(Resource):
    ...
    def get(self):
        ...
        alarm_outputs = AlarmOutput.query.all()
        alarm_outputs_schema = AlarmOutputSchema(many=True)
        return {'alarm_types': alarm_types_schema.dump(alarm_types), 'alarms': alarm_schema.dump(alarm), 'alarm_outputs': alarm_outputs_schema.dump(alarm_outputs) }
    def post(self):
