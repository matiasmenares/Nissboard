    ...
    <v-toolbar color="light-blue" dark>
      ...
      <v-spacer></v-spacer>
        <v-menu bottom left>
          ...
          <v-list>
            <v-list-item>
              <v-list-item-title @click="goto_channel()">New Analog</v-list-item-title>
            </v-list-item>
            <v-list-item>
              <v-list-item-title @click="goto_channel()">New Consult</v-list-item-title>
            </v-list-item>
            <v-list-item>
              <v-list-item-title @click="goto_channel()">New OBDII</v-list-item-title>
            </v-list-item>
            <v-list-item>
              <v-list-item-title @click="goto_channel()">New CAN</v-list-item-title>
            </v-list-item>
          </v-list>
        </v-menu>
    </v-toolbar>
    <v-list two-line subheader class="overflow-y-auto">
      <v-list-item v-for="item in items" :key="item.title" @click="set_channel(item)" >
        ...
        <v-list-item-content>
          ...
          <v-list-item-action>
            <v-list-item-action-text v-text="item.pin"></v-list-item-action-text>
          </v-list-item-action>
      </v-list-item>
    </v-list>
  </v-card>
...
<script>
  export default {
    ...
    methods:{
        set_list(){
          this.axios.get("/settings/channels/input").then(result => {
            result.data.channels.map(channel => {
              this.items.push({icon: 'mdi-current-ac', iconClass: 'blue white--text', messure: "Messure: "+(channel[5] == 1 ? "Voltage" : "Resistance"), title: channel[3], subtitle: 'Analog 5V', pin: "Pin: "+channel[4], channel: channel})
            })
          }).catch(error => {
            console.log(error);
          })
        ...
        goto_channel(){
          this.$router.push({ name: "setting-channel-analog-input"});
        }
    }
  }
</script>
