<template>
  ...
  <v-card>
    <v-tabs color="accent-4" left>
        ...
        <v-tab @click="hide" :disabled="this.form.channel_inputs_id == null || this.form.name == null">Data</v-tab>
        <v-tab @click="hide" :disabled="this.form.channel_inputs_id == null || this.form.name == null">Offset</v-tab>
        <v-tab @click="hide" :disabled="this.form.channel_inputs_id == null || this.form.name == null">Confirm</v-tab>
        <v-tab-item>
            <v-container fluid>
    ...
    methods:{
        ...
        set_inputs(){
            this.axios.get("/settings/channels/input").then(result => {
                this.input_value = result.data.nissan_inputs
            }).catch(error => {
              console.log(error);
            })
        },
        ...
        save(){
          if(this.channel){
            this.axios.patch("/settings/channels/output",{ channel: this.form, form_type: 3 }).then(() => {
              this.$router.push({ name: "setting-channel-output-list"});
            }).catch(error => {
              console.log(error);
            })
          }else{
            this.axios.post("/settings/channels/output",{ channel: this.form, form_type: 3  }).then(() => {
              this.$router.push({ name: "setting-channel-output-list"});
            }).catch(error => {
              console.log(error);
            })
